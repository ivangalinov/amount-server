...
Эндпоинты **`/category`**, **`/operation`** и **`/stats`** требуют той же cookie с JWT. Поле **`user_id` в теле запросов не используется** — автор записи берётся из сессии. Доступ только к workspace, где пользователь состоит в `workspace_users` (чужой workspace — `403` при создании категории и при запросе статистики; операции и категории из чужого workspace — ответ `404`).
...
| Метод | Путь | Описание |
|-------|------|----------|
| `GET` | `/stats/dashboard` | Query: **`workspace_id`** (обязательно); опционально `date_from`, `date_to` (ISO datetime). Показатели считаются в БД (`SUM` / `CASE` / `GROUP BY`), без выборки всех строк: `balance`, `total_income`, `total_expense`, `expenses_by_category`, `income_by_category`, `expenses_by_day`, `expenses_by_month`. Семантика как у клиента: доходы — `amount > 0`, расходы и срезы по дням/месяцам — `amount <= 0`. День и месяц — в **UTC** (PostgreSQL: `AT TIME ZONE 'UTC'`; SQLite в тестах: `date` / `strftime`). |
