...
Интеграционные тесты HTTP-роутов категорий: отдельное приложение FastAPI, изолированная БД в памяти, без поднятого PostgreSQL и без вызова `init_db()` из `main.py`.
...
| **`app`** | Минимальное `FastAPI()` только с роутером категорий. **`get_db`** подменяется через `dependency_overrides`: вместо боевого `AsyncSessionLocal` из `database.py` в запросы попадает сессия от тестового движка. Так тесты не трогают реальный `DATABASE_URL` и не требуют Postgres. |
